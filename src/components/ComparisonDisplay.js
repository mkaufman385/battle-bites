...
const ComparisonDisplay = ({ foodData }) => {
  // Destructure healthy and unhealthy foods directly
  const { healthyFood = {}, unhealthyFood = {} } = foodData || {};

  // Helper function to get nutritional values safely
  const getNutritionalValues = (food) => {
    // Check if the food object and its servings are valid
    if (!food || !food.servings || !food.servings.serving) return {};
    ...
    const servings = Array.isArray(food.servings.serving)
      ? food.servings.serving
      : [food.servings.serving]; // Ensure we always work with an array

    const serving = servings[0]; // Take the first serving (if available)
